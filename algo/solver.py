...
class BOOMExplorerSolver(object):
    ...
    def initialize(self) -> NoReturn:
        ...
        self.visited_x, self.visited_y, self.visited_time = micro_al(self.problem)
        self.adrs = []
    ...
    def fit_time_reg(self, verbose=False) -> NoReturn:
        from time_regression import TimeRegression
        self.time_reg = TimeRegression(verbose=verbose)
        self.time_reg.train_time_reg(self.visited_x[:,1].to(torch.float).view(-1,1), self.visited_time.to(torch.float).view(-1,1))
    ...
    def eipv_suggest(self, batch: int = 1) -> NoReturn:
        ...
        ## 加上time
        fit_time = self.time_reg.evaluate_time(self.problem.x[:,1].to(torch.float).view(-1,1)).squeeze()
        weight_time = 1e-6
        acq_val = acq_val + weight_time * fit_time.to(torch.float).to(self.model.device)
        ...
        self.visited_time = torch.cat((
                self.visited_time,
                self.problem.evaluate_time(new_x).unsqueeze(0)
            ),
            0
        )

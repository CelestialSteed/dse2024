...
class DesignSpaceProblem(MultiObjectiveProblem):
    ...
    def evaluate_time(self, x: torch.Tensor) -> torch.Tensor:
        assert self.configs["mode"] == "offline"
        _, indices = torch.topk(
            ((self.x.t() == x.unsqueeze(-1)).all(dim=1)).int(),
            1,
            1
        )
        return self.time[indices].to(torch.float32).squeeze()
    ...
    def remove_sampled_data(self, x: torch.Tensor) -> NoReturn:
        if self.configs["mode"] == "offline":
            ...
            self.time = self.time[mask[:] == False]     # 同时移除对应的 time 数据
